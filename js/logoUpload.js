$(function(){
	...
	$('#upload').on('change', function () {
		if (upload.files.length == 0) { //按"取消"時不做任何動作
			return;
		} else {
			uploadFiles();
			$('.upload-wrapper').removeClass('dragging');
...
function checkFile(i, action) {
	...
	if (validExts.indexOf(fileExt) < 0) {
		...
		if (file.size < (maxSize * 1000 * 1024)) {  //符合條件 會執行預設動作
			if (action === 'add') {  //增加檔案時 額外添加
...
function removeFile(i) {
	...
	for (let file of upload.files) { // This will remove the [i] item
...
function updateInfo() {
	...
	if (upload.value !== '') {
		...
		$('.upload-wrapper').addClass('bg-light align-content-between')
							.removeClass('align-content-center');
